...
void combo_sheck_h(int y, int x, int* col);
void combo_check_w(int y, int x, int* col);
...
int StageInitialize(void)
{
	...
	//画像読み込み
	LoadDivGraph("imaes/block.png", BLOCK_IMAGE_MAX, BLOCK_IMAGE_MAX, 1, BLOCKSIZE, BLOCKSIZE, BlockImage);
...
void SelecBlock(void)
{
	...
	if (Select[SELECT_CURSOR].x > WIDTE - 3)
	{
...
1][Select[NEXT_CURSOR].x + 1].image;
			Block[Select[NEXT_CURSOR].y + 1][Select[TMP_CURSOR].x +
...
1].image = TmpBlock;
		...
		{
			//連鎖が３つ以上ならブロック移動処理へ以降する
			Stage_State = 1;
		}

		//次にkリックできるようにClockFlegを0にする
		ClickStatus = E_NONE;
	}
}
